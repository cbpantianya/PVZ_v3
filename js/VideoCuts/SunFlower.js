...
class SunFlower {
    ...
    __init__(x, y) {
        ...
        var peaShooterSheet = new createjs.SpriteSheet({
            ...
            "frames": { "width": 73, "height": 74, "count": 18, "regX": 0, "regY": 0, "spacing": 0, "margin": 0 },
            "animations": {
...
class SunFlowerCard {
    ...
    mousemove(PeaShooterInHand, e) {
        ...
        if (minDistance <= 50 && this.__plantInHand__) {
            ...
            if (window.stage.getChildByName("gameContainer").getChildByName("vituralSunFlower")) {
                ...
                // 生成一个新的射手，但是半透明
                var vituralPeaShooter = new SunFlower(minLand.center.x, minLand.center.y)
    ...
    plantPlant() {
        // 种植物
        // 在指定位置生成一个豌豆射手
        var peaShooter = new SunFlower(window.stage.getChildByName("gameContainer").getChildByName("vituralSunFlower").x, window.stage.getChildByName("gameContainer").getChildByName("vituralSunFlower").y)
