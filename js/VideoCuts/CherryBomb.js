class CherryBomb {
  ...
  startBomb() {
    ...
    setTimeout(
      function () {
        ...
        var img = new createjs.Bitmap(
          window.loader.getResult("CherryBombBoom")
        );
    ...
    window.zombieList.forEach((e) => {
      if (
        Math.abs(e.zombie.x - this.peaShooter.x + 100) < 120 &&
        Math.abs(e.zombie.y - this.peaShooter.y + 80) < 150
      ) {
        (function (item) {
          setTimeout(function () {
            e.boom();
  ...
  mousemove(PeaShooterInHand, e) {
    ...
    if (minDistance <= 50 && this.__plantInHand__) {
      ...
      if (
        ...
        // 生成一个新的射手，但是半透明
        var vituralPeaShooter = new CherryBomb(
          minLand.center.x,
          minLand.center.y
        );
